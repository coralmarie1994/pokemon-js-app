...
let pokemonRepository = (function () {
  let pokemonList = [];
  let apiUrl = 'https://pokeapi.co/api/v2/pokemon/?offset=51&limit=150';
  ...
  function loadList() {
    //this is the fetch for API pokemon list
    return fetch(apiUrl)
      ...
      .then(function (json) {
        json.results.forEach(function (item) {
          //creates array with name & url for each one by one
          let pokemon = {
      ...
      .catch(function (e) {
        //will catch any errors if the fetch fails
        console.error(e);
      });
  }
 ...
 //pulls the image from the API
  ...
  function showDetails(pokemon) {
    loadDetails(pokemon).then(function () {
      ...
      modalHeight.text(`Height: ${(pokemon.height / 10).toFixed(1)} m`);
      modalWeight.text(`Weight: ${(pokemon.weight / 10).toFixed(1)} kg`);
...
let searchName=$('#search-input').val().trim().toUpperCase();
let searchList=pokemonRepository.getAll();
let groupList=$('.pokemon-list');

groupList.empty();

let filterList = searchList.filter(function (pokemon){
  return pokemon.name.toUpperCase().includes(searchName);
})
...
groupList.append($('<li class="list-group-item text-center h3 text-dark">No Pokemon Located </li>'))}
});
